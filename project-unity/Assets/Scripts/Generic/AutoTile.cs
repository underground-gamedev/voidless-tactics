...
namespace Assets.Scripts.Generic
{
    ...
    public class AutoTile : Tile
    {
        ...
        public override void RefreshTile(Vector3Int location, ITilemap tilemap)
        {
            for (int yd = -1; yd <= 1; yd++)
                for (int xd = -1; xd <= 1; xd++)
                {
                    ...
                    if (HasAutoTile(tilemap, position))
        ...
        {
            int mask = HasAutoTile(tilemap, location + new Vector3Int(0, 1, 0)) ? 1 : 0;
            mask += HasAutoTile(tilemap, location + new Vector3Int(1, 0, 0)) ? 2 : 0;
            mask += HasAutoTile(tilemap, location + new Vector3Int(0, -1, 0)) ? 4 : 0;
            mask += HasAutoTile(tilemap, location + new Vector3Int(-1, 0, 0)) ? 8 : 0;
            ...
            {
                Debug.LogWarning("Not enough sprites in AutoTile instance");
            }
        }
