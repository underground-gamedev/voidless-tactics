...
namespace Battle
{
    public class ManaManipulatorComponent : MonoBehaviour, IManaContainerComponent, IManaPickupComponent, IRoundWatcher
    {
        ...
        public bool ManaPickupAvailable(TacticMap map, MapCell src)
        {
            return ManaPickupAvailable(map) && src.Mana.ManaType != ManaType.None;
        }

        public bool ManaPickupAvailable(TacticMap map)
        {
            return !pickupUsed;
        }

        public IEnumerator ManaPickup(TacticMap map, MapCell src)
        {
